...
// VAQTINCHALIK — ishlab chiqarish kiritishlarini Excel qilib olib berish (tekshiruvdan keyin O'CHIRILADI).
app.get('/api/_diag/diag-3dd6a25557cbd695/prod-xlsx', async (req, res) => {
  try {
    const { query } = require('./db');
    const ExcelJS = require('exceljs');
    let iconv = null; try { iconv = require('iconv-lite'); } catch (e) {}
    const start = req.query.start || '2026-07-01';
    const end = req.query.end || '2026-07-25';
    const doFix = req.query.fix === '1';
    // Mojibake tuzatish: UTF-8 baytlari Windows-1251 sifatida o'qib qolingan bo'lsa qaytaradi
    const fix = (s) => {
      if (!s || !doFix || !iconv) return s;
      try { return iconv.decode(iconv.encode(s, 'win1251'), 'utf8'); } catch (e) { return s; }
    };
    const r = await query(`
      SELECT ep.production_date, e.name AS employee_name, e.type AS employee_type,
             p.name AS product_name, ep.rang, ep.quantity_produced, ep.daily_tariff,
             ep.calculated_amount, ep.production_type, ep.approval_status
      FROM employee_production ep
      JOIN employees e ON ep.employee_id = e.id
      LEFT JOIN products p ON ep.product_id = p.id
      WHERE ep.production_date BETWEEN $1 AND $2
      ORDER BY ep.production_date, e.name, p.name`, [start, end]);

    const wb = new ExcelJS.Workbook();
    wb.creator = 'TEKNOPLAST';
    const sh = wb.addWorksheet('Ishlab chiqarish');
    sh.columns = [
      { header: 'SANA', key: 'date', width: 12 },
      { header: 'XODIM', key: 'emp', width: 26 },
      { header: 'TUR', key: 'etype', width: 12 },
      { header: 'MAHSULOT', key: 'prod', width: 34 },
      { header: 'RANG', key: 'rang', width: 12 },
      { header: 'TAYYOR/YARIM', key: 'ptype', width: 14 },
      { header: 'DONA', key: 'qty', width: 12 },
      { header: 'TARIF', key: 'tariff', width: 12 },
      { header: 'SUMMA', key: 'sum', width: 16 },
      { header: 'HOLAT', key: 'status', width: 14 },
    ];
    sh.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } };
    sh.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1D4ED8' } };
    r.rows.forEach((x) => {
      sh.addRow({
        date: x.production_date,
        emp: fix(x.employee_name) || '—',
        etype: x.employee_type || '—',
        prod: fix(x.product_name) || '—',
        rang: fix(x.rang) || '—',
        ptype: x.production_type === 'SEMI_FINISHED' ? 'Yarim' : (x.production_type === 'KOMPONENT' ? 'Komponent' : 'Tayyor'),
        qty: parseFloat(x.quantity_produced || 0),
        tariff: parseFloat(x.daily_tariff || 0),
        sum: parseFloat(x.calculated_amount || 0),
        status: x.approval_status === 'APPROVED' ? 'Tasdiqlangan' : (x.approval_status === 'REJECTED' ? 'Qaytarilgan' : 'Kutilmoqda'),
      });
    });
    const tot = sh.addRow({
      prod: 'JAMI:',
      qty: r.rows.reduce((a, x) => a + parseFloat(x.quantity_produced || 0), 0),
      sum: r.rows.reduce((a, x) => a + parseFloat(x.calculated_amount || 0), 0),
    });
    tot.font = { bold: true };

    const buf = await wb.xlsx.writeBuffer();
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="ishlab-chiqarish-${start}_${end}.xlsx"`);
