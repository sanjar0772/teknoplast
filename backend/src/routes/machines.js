...
router.post('/', requireRole('OWNER', 'PRODUCTION_HEAD'), [
  ...
  try {
    ...
    const { name, status, operator_id, last_service_date, next_service_date, daily_production_capacity, location } = req.body;
    const result = await query(
      'INSERT INTO machines (name, status, operator_id, last_service_date, next_service_date, daily_production_capacity, location) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *',
      [name, status || 'WORKING', operator_id || null, last_service_date, next_service_date, daily_production_capacity || 0, location]
    );
...
router.put('/:id', requireRole('OWNER', 'PRODUCTION_HEAD'), async (req, res, next) => {
  try {
    const { name, status, operator_id, last_service_date, next_service_date, daily_production_capacity, location } = req.body;
    const result = await query(
      'UPDATE machines SET name=$1,status=$2,operator_id=$3,last_service_date=$4,next_service_date=$5,daily_production_capacity=$6,location=$7,updated_at=NOW() WHERE id=$8 RETURNING *',
      [name, status, operator_id, last_service_date, next_service_date, daily_production_capacity, location, req.params.id]
    );
